...
set -o nounset
set -o allexport
readonly githubRepository='dandavison/delta'
readonly binaryName='delta'
...
install() {
    ...
    readonly downloadUrlTemplate='https://github.com/${githubRepository}/releases/download/${releaseTag}/${binaryName}-${releaseTag}-${architecture}-unknown-linux-musl.tar.gz'
    readonly downloadUrl="$(echo -n "$downloadUrlTemplate" | envsubst)"
    curl_check_url "$downloadUrl"
    readonly binaryPathInArchiveTemplate='${binaryName}-${releaseTag}-${architecture}-unknown-linux-musl/${binaryName}'
