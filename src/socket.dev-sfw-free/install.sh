...
set -o nounset
set -o allexport
readonly githubRepository='SocketDev/sfw-free'
readonly binaryName='sfw'
...
install() {
    ...
    readonly downloadUrl="https://github.com/${githubRepository}/releases/download/${releaseTag}/sfw-free-linux-${architecture}"
    curl_check_url "$downloadUrl"
    readonly binaryTargetPath="${binaryTargetFolder}/${binaryName}"
    curl_download_binary "$downloadUrl" "$binaryTargetPath"
