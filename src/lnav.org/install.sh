...
set -o nounset
set -o allexport
readonly githubRepository='tstack/lnav'
readonly binaryName='lnav'
...
curl_download_unzip() {
    local url=$1
    local binary_in_archive=$2
    local target=$3
    local temp_file
    temp_file=$(mktemp --suffix='.zip')
    curl_download_stdout "$url" >| "$temp_file"
    unzip -p "$temp_file" "$binary_in_archive" >| "${target}/$(basename "$binary_in_archive")"
    rm -f "$temp_file"
}
debian_get_arch() {
...
install() {
    ...
    readonly downloadUrlTemplate='https://github.com/${githubRepository}/releases/download/${releaseTag}/lnav-${version}-linux-musl-${architecture}.zip'
    readonly downloadUrl="$(echo -n "$downloadUrlTemplate" | envsubst)"
    curl_check_url "$downloadUrl"
    readonly binaryPathInArchive="lnav-${version}/lnav"
    readonly binaryTargetPath="$(echo -n "$binaryTargetPathTemplate" | envsubst)"
    curl_download_unzip "$downloadUrl" "$binaryPathInArchive" "$binaryTargetFolder"
