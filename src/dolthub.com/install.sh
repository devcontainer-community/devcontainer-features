...
set -o nounset
set -o allexport
readonly githubRepository='dolthub/dolt'
readonly binaryName='dolt'
...
install() {
    ...
    readonly downloadUrlTemplate='https://github.com/${githubRepository}/releases/download/${releaseTag}/dolt-linux-${architecture}.tar.gz'
    readonly downloadUrl="$(echo -n "$downloadUrlTemplate" | envsubst)"
    curl_check_url "$downloadUrl"
    readonly binaryPathInArchive="dolt-linux-${architecture}/bin/${binaryName}"
