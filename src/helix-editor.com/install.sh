...
set -o nounset
set -o allexport
readonly githubRepository='helix-editor/helix'
readonly binaryName='hx'
readonly binaryTargetFolder='/usr/local/bin'
readonly helixInstallDir='/usr/local/lib/helix'
...
github_list_releases() {
    ...
    curl -s "$url" | grep -Po '"tag_name": "\K.*?(?=")' | grep -E '^[0-9]+\.[0-9]+(\.[0-9]+)?$'
}
...
github_get_tag_for_version() {
    ...
    curl -s "$url" | grep -Po '"tag_name": "\K.*?(?=")' | grep -E "^${escaped_version}$" | head -n 1
}
utils_check_version() {
    local version=$1
    if ! [[ "${version:-}" =~ ^(latest|[0-9]+\.[0-9]+(\.[0-9]+)?)$ ]]; then
        printf >&2 '=== [ERROR] Option "version" (value: "%s") is not "latest" or valid version format "YY.MM" or "YY.MM.patch" !\n' \
            "$version"
        exit 1
    fi
}
install() {
    ...
    readonly downloadUrl="https://github.com/${githubRepository}/releases/download/${releaseTag}/helix-${releaseTag}-${architecture}-linux.tar.xz"
    curl_check_url "$downloadUrl"
    mkdir -p "$helixInstallDir"
    curl_download_stdout "$downloadUrl" | tar -xJ -f '-' --strip-components=1 -C "$helixInstallDir"
    chmod 755 "${helixInstallDir}/${binaryName}"
    ln -sf "${helixInstallDir}/${binaryName}" "${binaryTargetFolder}/${binaryName}"
    apt_get_cleanup
}
