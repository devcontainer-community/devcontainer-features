...
set -o nounset
set -o allexport
readonly githubRepository='dolthub/doltgresql'
readonly binaryName='doltgres'
...
install() {
    ...
    readonly downloadUrlTemplate='https://github.com/${githubRepository}/releases/download/${releaseTag}/doltgresql-linux-${architecture}.tar.gz'
    readonly downloadUrl="$(echo -n "$downloadUrlTemplate" | envsubst)"
    curl_check_url "$downloadUrl"
    readonly binaryPathInArchiveTemplate='doltgresql-linux-${architecture}/bin/${binaryName}'
