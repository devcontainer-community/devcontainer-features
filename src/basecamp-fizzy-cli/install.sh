...
set -o nounset
set -o allexport
readonly githubRepository='basecamp/fizzy-cli'
readonly binaryName='fizzy'
...
install() {
    ...
    readonly downloadUrlTemplate='https://github.com/${githubRepository}/releases/download/${releaseTag}/fizzy-linux-${architecture}'
    readonly downloadUrl="$(echo -n "$downloadUrlTemplate" | envsubst)"
    curl_check_url "$downloadUrl"
    curl_download_file "$downloadUrl" "$binaryTargetPath"
